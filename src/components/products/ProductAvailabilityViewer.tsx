...
import { checkProductAvailabilityForRental } from '@/lib/rental-availability';
...
export function ProductAvailabilityViewer({ lang }: ProductAvailabilityViewerProps) {
  ...
  const [checkDeliveryDate, setCheckDeliveryDate] = useState<Date | undefined>(undefined);
  const [checkReturnDate, setCheckReturnDate] = useState<Date | undefined>(undefined);
  const [availabilityResult, setAvailabilityResult] = useState<{ available: boolean; conflictingOrders: any[] } | null>(null);
  ...
  const t = {
    ...
    checkAvailabilityForDates: lang === 'ar' ? 'التحقق من التوفر لتواريخ محددة' : 'Check Availability for Specific Dates',
    deliveryDateLabel: lang === 'ar' ? 'تاريخ التسليم' : 'Delivery Date',
    returnDateLabel: lang === 'ar' ? 'تاريخ الإرجاع' : 'Return Date',
    checkAvailability: lang === 'ar' ? 'التحقق من التوفر' : 'Check Availability',
    checking: lang === 'ar' ? 'جاري التحقق...' : 'Checking...',
    availableForSelectedDates: lang === 'ar' ? 'متاح للتواريخ المحددة' : 'Available for Selected Dates',
    notAvailableForSelectedDates: lang === 'ar' ? 'غير متاح للتواريخ المحددة' : 'Not Available for Selected Dates',
    conflictingRentals: lang === 'ar' ? 'إيجارات متضاربة' : 'Conflicting Rentals',
    selectBothDates: lang === 'ar' ? 'يرجى تحديد تاريخ التسليم وتاريخ الإرجاع' : 'Please select both delivery and return dates',
    returnDateMustBeAfterDelivery: lang === 'ar' ? 'تاريخ الإرجاع يجب أن يكون بعد تاريخ التسليم' : 'Return date must be after delivery date',
  };
  ...
  const handleCheckAvailability = async () => {
    if (!productDetails) {
      setError(lang === 'ar' ? 'يرجى البحث عن منتج أولاً' : 'Please search for a product first');
      return;
    }

    if (!checkDeliveryDate || !checkReturnDate) {
      setError(t.selectBothDates);
      return;
    }

    if (checkReturnDate <= checkDeliveryDate) {
      setError(t.returnDateMustBeAfterDelivery);
      return;
    }
    ...
    try {
      const deliveryDateStr = checkDeliveryDate.toISOString().split('T')[0];
      const returnDateStr = checkReturnDate.toISOString().split('T')[0];
      ...
      setError(e.message || (lang === 'ar' ? 'حدث خطأ أثناء التحقق من التوفر.' : 'An error occurred while checking availability.'));
    } finally {
  ...
  return (
    <Card className="shadow-lg rounded-lg">
      ...
      <CardContent className="space-y-6">
        ...
        {/* Date Availability Check Section */}
        {productDetails && productDetails.category === 'Rental' && (
          <Card className="border-2 border-dashed border-primary/20">
            <CardHeader>
              <CardTitle className="text-lg flex items-center">
                <CalendarCheck className="mr-2 h-5 w-5 text-primary rtl:ml-2 rtl:mr-0" />
                {t.checkAvailabilityForDates}
              </CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-2">
                  ...
                  <DatePicker
                    date={checkDeliveryDate}
                    setDate={setCheckDeliveryDate}
                    lang={lang}
                    placeholder={t.deliveryDateLabel}
                  />
                </div>
                <div className="space-y-2">
                  <label className="text-sm font-medium">{t.returnDateLabel}</label>
                  <DatePicker
                    date={checkReturnDate}
                    setDate={setCheckReturnDate}
                    lang={lang}
                    placeholder={t.returnDateLabel}
                    disabled={(date) => {
                      return checkDeliveryDate ? date <= checkDeliveryDate : false;
                    }}
                  />
                </div>
              </div>
              ...
              <Button
                ...
                disabled={isCheckingAvailability || !checkDeliveryDate || !checkReturnDate}
                className="w-full"
              >
                {isCheckingAvailability ? (
                  <>
                    <Loader className="mr-2 h-4 w-4 animate-spin rtl:ml-2 rtl:mr-0" /> {t.checking}
                  </>
                ) : (
                  <>
                    <CalendarCheck className="mr-2 h-4 w-4 rtl:ml-2 rtl:mr-0" /> {t.checkAvailability}
                  </>
                )}
              </Button>
                ...
                <div className={`p-4 rounded-md border ${
                  ...
                  <div className="flex items-center">
                    ...
                    <span className="font-medium">
                      {availabilityResult.available ? t.availableForSelectedDates : t.notAvailableForSelectedDates}
                    </span>
                  </div>

                  {!availabilityResult.available && availabilityResult.conflictingOrders.length > 0 && (
                    <div className="mt-3">
                      <p className="font-medium mb-2">{t.conflictingRentals}:</p>
                      <ul className="space-y-1 text-sm">
                        {availabilityResult.conflictingOrders.map((order, index) => (
                          <li key={index} className="flex items-center justify-between">
                            <span>
                              {order.customerName || 'Unknown Customer'} - {order.orderCode || order.id}
                            </span>
