...
import { LogIn, LogOut, Users as UsersIcon, BarChartHorizontalBig, Banknote, LayoutDashboard, ShoppingBag, Users, ListOrdered, Shirt, Feather, Undo2, UserCircle, Store, PackageSearch, CalendarCheck, Menu, X, Settings } from 'lucide-react';
...
const SiteHeaderComponent = ({ lang }: SiteHeaderProps) => {
  ...
  const navLinks: NavLinkItem[] = React.useMemo(() => [
    ...
    { href: "/settings", en: "Settings", ar: "الإعدادات", icon: Settings, requiredPermission: 'users_manage' },
  ], []);
  ...
  const filteredNavLinks = React.useMemo(() => {
    ...
    return navLinks.filter(link => {
      ...
      switch (link.href) {
        ...
        case "/settings": return checkPermission.has('users_manage');
        default: return userHasPermissionForLink(link);
      }
    });
  }, [navLinks, isLoading, isAuthenticated, commonPermissions, userHasPermissionForLink, checkPermission]);
