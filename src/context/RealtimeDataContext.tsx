...
import type { Product, Order, Customer, User, FinancialTransaction, Branch, ProductTypeDefinition } from '@/types';
...
export function RealtimeDataProvider({ children, enableNotifications = true }: RealtimeDataProviderProps) {
  ...
  useEffect(() => {
    ...
    const collections = [
      ...
      { name: 'product_types', stateKey: 'productTypes' as const },
    ];
  ...
  const refreshData = useCallback((collection?: keyof Omit<RealtimeDataState, 'isLoading' | 'connectionStatus' | 'lastUpdated'>) => {
    if (collection) {
      ...
      const collectionName = collection === 'financialTransactions' ? 'financial_transactions' :
                            collection === 'productTypes' ? 'product_types' : collection;
