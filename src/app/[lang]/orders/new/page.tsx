...
import { checkMultipleProductsAvailability } from '@/lib/rental-availability';
...
export default function AddNewOrderPage() {
  ...
  const onSubmit: SubmitHandler<FormData> = async (formData) => {
    ...
    try {
      ...
      // Check for rental date conflicts if this is a rental order
      if (formData.transactionType === 'Rental' && formData.deliveryDate && formData.returnDate) {
        ...
        if (!availabilityCheck.available) {
          const conflictMessages = availabilityCheck.conflicts.map(conflict => {
            const productName = conflict.productName || conflict.productId;
            const conflictDates = conflict.conflictingOrders.map(order =>
              `${order.deliveryDate} - ${order.returnDate}`
            ).join(', ');
            return effectiveLang === 'ar'
              ? `${productName}: متاح في التواريخ (${conflictDates})`
              : `${productName}: conflicting dates (${conflictDates})`;
          });

          const errorMessage = effectiveLang === 'ar'
            ? `المنتجات التالية غير متاحة في التواريخ المحددة:\n${conflictMessages.join('\n')}`
            : `The following products are not available for the selected dates:\n${conflictMessages.join('\n')}`;

          toast({
            title: effectiveLang === 'ar' ? 'تضارب في تواريخ الإيجار' : 'Rental Date Conflict',
